...
export function mountStubStudio(
  ...
  root.className = ["tfs-studio", options.className ?? ""]
    .filter(Boolean)
    .join(" ");
  ...
  content.append(
    scenariosPanel.section,
    storagePanel.section,
    mappingPanel.section,
  );
...
function renderHeader(
  ...
  if (advancedModeStore) {
    const advancedWrap = createToggleSwitch(
      "Advanced user",
      "tfs-header-toggle",
    );
  ...
  if (autoRefreshStore) {
    const autoRefreshWrap = createToggleSwitch(
      "Auto refresh",
      "tfs-header-toggle",
    );
  ...
  if (
    ...
    if (documentationLinks && documentationLinks.length > 0) {
      documentationLinks.forEach((link) => {
        ...
        linkEl.rel =
          (link.target ?? "_blank") === "_blank" ? "noopener noreferrer" : "";
      ...
      guideLink.rel =
        documentationTarget === "_blank" ? "noopener noreferrer" : "";
...
function renderStoragePanel(
  ...
  manualKeyInput.placeholder =
    "Type key without prefix, e.g. resource:/profile";
  ...
  const getActiveKey = (): string => {
    ...
    if (advanced) {
      ...
      if (manualKey) {
        return manualKey.startsWith("fabricate:")
          ? manualKey
          : `fabricate:${manualKey}`;
      }
    } else {
  ...
  const renderSimpleRows = () => {
    ...
    simpleRows.forEach((row, index) => {
      ...
      const types: Array<StorageSimpleValueType> = [
        "string",
        "number",
        "boolean",
        "null",
      ...
      removeButton.innerHTML =
        '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none"><path stroke="currentColor" stroke-width="2" d="M4 4l8 8M12 4l-8 8"/></svg>';
...
function renderMappingPanel(
  ...
  addFieldButton.addEventListener("click", () => {
    simpleRows = [
      ...simpleRows,
      { path: "", type: "number.int", options: "18,30,true" },
    ];
    renderSimpleRows();
  });
  ...
  removeRouteButton.addEventListener("click", () => {
    ...
    manager.setPostGeneratedFieldsJson(
      next as StubPostGeneratedFieldsJsonInput,
    );
    refresh();
  });
...
function buildStorageSimpleValue(
  rows: StorageSimpleRow[],
...
function buildSimpleRouteConfig(
